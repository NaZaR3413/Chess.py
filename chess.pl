...
play(Board) :-
%   removed the prompt to get command, switched to automated chess
    execute_command(playerA, Board, NewBoard),
    execute_command(playerB, NewBoard, NextNewBoard),
    play(NextNewBoard).

% method to execute commands for both player a and b
...
% Strength methods for player A based on B
...
% Defined utility function for playerA, based on playerB
% Sum being smaller than 32000
...
bookA( [ state(white, WhiteKing, WhiteKingRook, WhiteQueenRook), % e2e4
    ...
    piece(h-2, white, pawn  ), piece(e-4, white, pawn  ) ], e-2, e-4). % switched to make white's first move

% Code for alpha beta prunning
% Player is playerA, Turn is the player whose turn is to play
% Based off of the player B version, simply switched to A
...
% Based off player B version, switched for A
...
report_move(Color, Board, From_File-From_Rank, To_File-To_Rank, Rating) :-
	...
	write(', Rating: '),
	write(Rating), nl,
	draw_board(Board,8,8). % draws board after each move
...
print_board(Board) :-
	write(Board), nl.
% print_board not used, adjusted to take advantage of recitation help

% loop to draw symbols
drawSymbol(Symbol, 0).
drawSymbol(Symbol, N) :-
    N > 0,
    write(Symbol),
...
% helper function for chess board
draw_content_line(Board,Line,Col) :- drawSymbol(Line,1),draw_content_cell(Board,Line,Col),nl.


% called in order to show the updated move after each turn
...
draw_pair:- drawSymbol(' ',4), drawSymbol('a',1),
			...
			drawSymbol(' ',4), drawSymbol('d',1),
			drawSymbol(' ',4), drawSymbol('e',1),
			drawSymbol(' ',4), drawSymbol('f',1),
			drawSymbol(' ',4), drawSymbol('g',1),
			drawSymbol(' ',4), drawSymbol('h',1).		

% defines pairs of chess pieces
pair(a,8).
pair(b,7).
pair(c,6).
pair(d,5).
